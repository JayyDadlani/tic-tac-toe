...
function computer_turn() {
    ...
    idx = is_next_move_winning('O');

    if(idx[0] === -1 && idx[1] === -1) {
        idx = is_next_move_winning('X');
    }

    if(idx[0] !== -1 && idx[1] !== -1) {
        const computer_btn = document.getElementById(`btn${idx[0]}${idx[1]}`);
        computer_btn.textContent = 'O';
        computer_btn.style.color = '#80CEEF';
        computer_btn.removeEventListener('click', handle_click);
        board_array[idx[0]][idx[1]] = 'O';
        for(let i=0; i<empty_btn_array.length; i++) {
            if(empty_btn_array[i][0] === idx[0] && empty_btn_array[i][1] === idx[1]) {
                empty_btn_array.splice(i, 1);
                break;
            }
        }
...
function is_next_move_winning(who) {
    let count;
    // Checking row
    for(let i=0; i<3; i++) {
        count = 0;
        for(let j=0; j<3; j++) {
            if(board_array[i][j] === who) {
                count++;
            }
        }
        if(count === 2) {
            for(let j=0; j<3; j++) {
                if(board_array[i][j] !== who && board_array[i][j] === 0) {
                    return [i, j];
                } 
            }
        }
    ...
    for(let i=0; i<3; i++) {
        count = 0;
        for(let j=0; j<3; j++) {
            if(board_array[j][i] === who) {
                count++;
            }
        }
        if(count === 2) {
            for(let j=0; j<3; j++) {
                if(board_array[j][i] !== who && board_array[j][i] === 0) {
                    return [j, i];
                } 
            }
        }
    ...
    count = 0;
    for(let i=0; i<3; i++) {
        if(board_array[i][i] === who) 
            count++;
    }
    if(count === 2) {
        for(let i=0; i<3; i++) {
            if(board_array[i][i] !== who && board_array[i][i] === 0) {
                return [i, i];
            }
        }
    }
    count = 0;
    for(let i=0; i<3; i++) {
        if(board_array[i][2-i] === who) 
            count++;
    }
    if(count === 2) {
        for(let i=0; i<3; i++) {
            if(board_array[i][2-i] !== who && board_array[i][2-i] === 0) {
                return [i, 2-i];
            }
        }
    }
